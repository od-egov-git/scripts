...
def connect():
    try:
        conn = psycopg2.connect(database="digit", user="digit_readonly",
                            password="Readonly@123", host = "20.198.122.138")
    ...
    fsmdata.head
    fsmdata.to_csv('/home/saumya/Desktop/fsmDatamart.csv')

    print("Datamart exported. Please copy it using kubectl cp command to your required location.")

def accessToken():
    query = {'username':'EMP_BALASORE','password':'eGov@123','userType':'EMPLOYEE',"scope":"read","grant_type":"password"}
    query['tenantId']='pb.amritsar'
    response = requests.post("https://sujog-dev.odisha.gov.in//user/oauth/token",data=query, headers={
   "Connection":"keep-alive","content-type":"application/x-www-form-urlencoded", "origin":"https://sujog-dev.odisha.gov.in/","Authorization": "Basic ZWdvdi11c2VyLWNsaWVudDo="})
...
def locationApiCall(tenantid):
    ...
    response = requests.post("https://sujog-dev.odisha.gov.in/egov-location/location/v11/boundarys/_search",params = paramlist,json=body, headers={
       "Connection":"keep-alive","content-type":"application/json;charset=UTF-8", "origin":"https://sujog-dev.odisha.gov.in/"})
